    ...
    def load(self, flag):
        ...
        for file in os.listdir(self.price_dir):
            ...
            for idx in data_range:
                ...
                print(ticker,"= ticker, end_date = ", end_date)

                for i, seq_date in enumerate(self.daterange(start_date, end_date)):
                    ...
                    #     break
                    # break
                    print("seq_date = ", seq_date)


                if summary_all != "":
                    data = pd.concat([data, pd.DataFrame([{'ticker': ticker, 'summary': summary_all.rstrip(), 'target': target, 'date': end_date}])], ignore_index=True)
                
                # if(data_range.index(idx) + 1 == 4):
                #     break
                # break
            
            data.to_csv('Data/checkpoint.csv', index=False)
